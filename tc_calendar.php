...
class tc_calendar{
	...
	function writeScript(){
		...
		if($this->date_picker){
			$this->ReturnHTMLDate.="<div style=\"position: relative; z-index: $this->zindex; float: left;\">";
			...
			if($this->show_input){
				...
				$this->ReturnHTMLDate.=" <a href=\"javascript:toggleCalendar('".$this->objname."');\">";
				$this->writeDateContainer();
				$this->ReturnHTMLDate.="</a>";
			}			
			
			$this->ReturnHTMLDate.=" <a href=\"javascript:toggleCalendar('".$this->objname."');\">";						
			if(is_file($this->icon)){
				$this->ReturnHTMLDate.="<img src=\"".$this->icon."\" id=\"tcbtn_".$this->objname."\" name=\"tcbtn_".$this->objname."\" border=\"0\" align=\"absmiddle\" />";
			}else $this->ReturnHTMLDate.=$this->txt;				
			$this->ReturnHTMLDate.="</a>";
			
			$this->writeCalendarContainer();
			
			$this->ReturnHTMLDate.="</div>";
		}else{
			$this->writeCalendarContainer();
		}			
	}
	...
	function writeCalendarContainer(){
		...
		$this->ReturnHTMLDate.="<div id=\"div_".$this->objname."\" style=\"position:".$div_position.";visibility:".$div_display.";z-index:100;".$div_align."\" class=\"div_calendar calendar-border\">";
		$this->ReturnHTMLDate.="<IFRAME id=\"".$this->objname."_frame\" src=\"".$this->path."calendar_form.php".$paramStr."\" frameBorder=\"0\" scrolling=\"no\" allowtransparency=\"true\" width=\"100%\" height=\"100%\" style=\"z-index: 100;\"></IFRAME>";
	...
	function writeDay(){
		...
		$this->ReturnHTMLDate.="<select name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" onChange=\"javascript:tc_setDay('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcday\">";
		$this->ReturnHTMLDate.="<option value=\"00\">Day</option>";
		for($i=1; $i<=$total_days; $i++){
			$selected = ((int)$this->day == $i) ? " selected" : "";
			$this->ReturnHTMLDate.="<option value=\"".str_pad($i, 2 , "0", STR_PAD_LEFT)."\"$selected>$i</option>";
		}
		$this->ReturnHTMLDate.="</select> ";
	}
	
	//write the select box of months
	function writeMonth(){
		$this->ReturnHTMLDate.="<select name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" onChange=\"javascript:tc_setMonth('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcmonth\">";
		$this->ReturnHTMLDate.="<option value=\"00\">Month</option>";
		
		$monthnames = $this->getMonthNames();		
		for($i=1; $i<=sizeof($monthnames); $i++){
			$selected = ((int)$this->month == $i) ? " selected" : "";
			$this->ReturnHTMLDate.="<option value=\"".str_pad($i, 2, "0", STR_PAD_LEFT)."\"$selected>".$monthnames[$i-1]."</option>";
		}
		$this->ReturnHTMLDate.="</select> ";
	}
	...
	function writeYear(){
		...
		$this->ReturnHTMLDate.="<select name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" onChange=\"javascript:tc_setYear('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcyear\">";
		$this->ReturnHTMLDate.="<option value=\"0000\">Year</option>";
		...
		for($i=$year_end; $i>=$year_start; $i--){
			...
			$this->ReturnHTMLDate.="<option value=\"$i\"$selected>$i</option>";
		}
		$this->ReturnHTMLDate.="</select> ";
	}
	
	function eHidden($suffix, $value,$onchange="") {
		//echo $onchange."#";
		if($suffix) $suffix = "_".$suffix;
		$this->ReturnHTMLDate.="<input $onchange type=\"hidden\" name=\"".$this->objname.$suffix."\" id=\"".$this->objname.$suffix."\" value=\"".$value."\" />";
	}

	//write hidden components
	function writeHidden(){
		global $_DATA;
		$this->eHidden('', $this->getDate(),@$_DATA[$this->objname]['ONCHANGE']);
	...
	function writeDateContainer(){
		...
		$this->ReturnHTMLDate.="<span id=\"divCalendar_".$this->objname."_lbl\" class=\"date-tccontainer\">$dd</span>";
	}
